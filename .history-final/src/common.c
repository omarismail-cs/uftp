...
static const uint32_t crc32_table[256] = {
    ...
    0xddbbbc9d, 0xacbcf940, 0x32d86ce3, 0x45df5c75, 0xdcd60dcf, 0xabd13d59,
...
uint32_t uftp_crc32(const void *data, size_t len) {
    return uftp_crc32_final(uftp_crc32_update(UFTP_CRC32_INIT, data, len));
}

uint32_t uftp_file_crc32(const char *path) {
    FILE *fp = fopen(path, "rb");
    if (!fp) {
        uftp_log("cannot hash file: %s", path);
        return 0;
    }

    uint8_t buf[8192];
    uint32_t crc = UFTP_CRC32_INIT;
    size_t n;

    while ((n = fread(buf, 1, sizeof(buf), fp)) > 0) {
        crc = uftp_crc32_update(crc, buf, n);
    }

    if (ferror(fp)) {
        fclose(fp);
        uftp_log("read error hashing: %s", path);
        return 0;
    }

    fclose(fp);
    return uftp_crc32_final(crc);
}
